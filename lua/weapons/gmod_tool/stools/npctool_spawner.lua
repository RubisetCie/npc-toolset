...
if(CLIENT) then
	...
	local function MainMenu(pnl)
		...
		npcz:AddColumn("NPC")
		npcz:SetMultiSelect(false)
		npcz:SetTall(150)
		npcz:SortByColumn(1,false)
		for k,v in pairs(options) do
			local line = npcz:AddLine(k)
			line.data = v
		...
		function npcz:OnRowSelected(LineID,Line)
			for k,v in pairs(Line.data) do
		...
		equipLbl:SetDark(true)
		equip:SetHeight(25)
		equip:Dock(TOP)
		pnl:AddItem(equipLbl,equip)
		...
		ptypesLbl:SetText("Patrol Type")
		ptypesLbl:SetDark(true)
		ptypes:SetHeight(25)
		ptypes:Dock(TOP)
		...
		kvals:AddColumn("Key / Values")
		kvals:SetMultiSelect(false)
		kvals:SetTall(150)
		kvals:SortByColumn(1,false)
		for k,v in pairs(options) do
			local line = kvals:AddLine(k)
			line.data = v
			for k,v in pairs(line.data) do
				if(GetConVarString(k) == tostring(v)) then
					line:SetSelected(true)
		...
		function kvals:OnRowSelected(LineID,Line)
			for k,v in pairs(Line.data) do
				RunConsoleCommand(k,v)
			end
		end
		...
		rlships:AddColumn("Relationships")
		rlships:SetMultiSelect(false)
		rlships:SetTall(150)
		rlships:SortByColumn(1,false)
		for k,v in pairs(options) do
			local line = rlships:AddLine(k)
			line.data = v
			for k,v in pairs(line.data) do
				if(GetConVarString(k) == tostring(v)) then
					line:SetSelected(true)
		...
		function rlships:OnRowSelected(LineID,Line)
			for k,v in pairs(Line.data) do
				RunConsoleCommand(k,v)
			end
		end
