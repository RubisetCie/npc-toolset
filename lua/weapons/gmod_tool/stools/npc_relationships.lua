...
TOOL.Name = "#tool.npc_relationships.name"
TOOL.Command = nil
TOOL.ConfigName = ""
...
if(CLIENT) then
	...
	language.Add("tool.npc_relationships.name","NPC Relationships")
	language.Add("tool.npc_relationships.desc","Change an NPC's relationship to another NPC or player")
	language.Add("tool.npc_relationships.0","Left-Click to select a source NPC or player, Right-Click to apply the relationship to the target you're looking at, Reload to set yourself as target.")
	function TOOL.BuildCPanel(pnl)
		...
		pCBox.OnSelect = function(pCBox,idx,val,data) RunConsoleCommand("npc_relationships_disposition",data) end
		pCBox:SetWide(130)
		pnl:AddItem(lbl,pCBox)
		pnl:CheckBox("Revert","npc_relationships_revert")
	end
	local tbSelected = {}
	net.Receive("npc_rel_add",function(len)
	...
	net.Receive("npc_rel_set",function(len)
		...
		local cvDisp = GetConVar("npc_relationships_disposition")
		local cvRevert = GetConVar("npc_relationships_revert")
	...
	net.Receive("npc_relationships_deploy",function(len)
	...
	net.Receive("npc_relationships_holster",function(len)
		for _,ent in ipairs(tbSelected) do
			if(ent:IsValid()) then ent:StopParticles() end
		end
	end)
else
	util.AddNetworkString("npc_rel_add")
	util.AddNetworkString("npc_rel_set")
	util.AddNetworkString("npc_relationships_holster")
	util.AddNetworkString("npc_relationships_deploy")
	function TOOL:Deploy()
		net.Start("npc_relationships_deploy")
...
function TOOL:Holster()
	...
	net.Start("npc_relationships_holster")
	net.Send(self:GetOwner())
end